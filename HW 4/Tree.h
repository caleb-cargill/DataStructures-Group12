...
public:
	...
	// Returns an int for the number of spots checked when adding the item
	int Insert(T value) {
	...
	T Remove(T key) {
		...
		if (temp->left != nullptr && temp->left->data == key) { // If the key node is on the left of the parent
			...
			if (delNode->left == nullptr && delNode->right == nullptr) { // Removing leaf
				...
				nodeCount++;
				if (temp->left != nullptr) delNode->left = temp->left->left;
				if (temp->left != nullptr) delNode->right = temp->left->right;
				delete temp->left;
				temp->left = delNode;
				if (delNode->left != nullptr) delNode->left->parent = delNode;
				if (delNode->right != nullptr) delNode->right->parent = delNode;
				delNode->parent = temp;
			}
			...
			if (delNode->left == nullptr && delNode->right == nullptr) { // Removing leaf
				...
				delNode = new Node(Remove(delNode->data));
				nodeCount++;
				if (temp->right != nullptr) delNode->left = temp->right->left;
				if (temp->right != nullptr) delNode->right = temp->right->right;
				delete temp->right;
				temp->right = delNode;
				if (delNode->left != nullptr) delNode->left->parent = delNode;
				if (delNode->right != nullptr) delNode->right->parent = delNode;
	...
	// Returns remove spot count
	int RemoveCount() {
		int temp = removeSpotCount;
		removeSpotCount = 0;
		return temp;
	}
