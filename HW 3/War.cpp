...
int main() {
	...
	// Play until player a player is out of cards
	if (gamemode == '1') {
		...
		while (!gameover) {
			...
			// Player has to play from side pile
			else {
		...
		if ((playerDeck.Length() == 0 && playerSide.Length() == 0)) {
			cout << endl << "Sorry, the computer won this game. Better luck next time" << endl;
		}
		else {
			cout << endl << "Congratulations! You beat the computer" << endl;
		}
	}
...
// Changes prompt based on side pile status
char getSideDecision(Side<Card> playerSide, Deck<Card> playerDeck) {
	char input;
	if (playerSide.isEmpty() && playerDeck.Length() > 0) {
		...
		while (input != '1' && input != '3') {
			cout << "Sorry that was not a valud option. Try again" << endl << endl;
			cout << "Enter a 1 to place the card onto the side pile" << endl;
			cout << "Enter a 3 to play the current card" << endl << endl;
			cout << "Input: ";
			cin >> input;
